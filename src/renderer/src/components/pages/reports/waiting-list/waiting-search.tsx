...
const WaitingSearch = () => {
  ...
  const { data: applicant } = useQuery({
    queryKey: ['applicantsReportCategorySearch'],
    queryFn: () => getApi<data[]>('/applicant/applicantsReportCategory')
  })
  const loadOptions = async (value: string) => {
    if (!value) return []
    const data = await getApi<data[]>('/applicant/applicantsReportCategory', {
      params: {
        'name[contains]': value
      }
    })
