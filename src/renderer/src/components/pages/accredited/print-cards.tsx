  ...
  return (
    <div ref={ref} className="w-[210mm] h-[297mm] p-5 mx-auto">
      ...
      <div className="grid grid-cols-3 grid-rows-4 gap-4">
        {Cards?.data.slice(0, 12).map((data, index) => <Card key={index} value={data.number} />)}
      </div>
    </div>
  )
})
...
const Card = ({ value }) => {
  return (
    <div className="bg-white border min-h-44 border-gray-300 p-4 flex flex-col items-center justify-center">
      ...
      <h1 className="my-1">رقم الاستمارة : 1515545</h1>
      <Barcode value={value} displayValue={false} fontSize={20} />
    </div>
  )
}
