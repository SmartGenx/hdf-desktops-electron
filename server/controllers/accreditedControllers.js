...
class AccreditedController {
	...
	async filterAccredited(req, res, next) {
		try {
			...
			const accreditedRecords = await AccreditedService.filterAccreditedByDateAndLocation(squareIdv, location, docter, state);
	...
	async createAccreditation(req, res, next) {
		...
		try {
			...
			const newAccreditation = await AccreditedService.createAccreditation(AccreditedData, fileAtch, filePt);
	...
	async exportAllBarcodeCardToPDFById(req, res, next) {
		try {
			...
			const htmlContent = await ejs.renderFile(templatePath, { accredited, barcodeImageSrc, logoImageSrc });
	...
	async exportAllBarcodeCardToPDF(req, res, next) {
		try {
			...
			for (const data of accrediteds) {
				...
				const cardInstance = await ejs.renderFile(templatePath, { data, barcodeImageSrc, logoImageSrc });
	...
	async exportAllAccreditedsToPDF(req, res, next) {
		try {
			...
			const htmlContent = await ejs.renderFile(fullPath, {
				accrediteds: accrediteds,logoImageSrc:logoImageSrc,logoImageSrc2:logoImageSrc2
			});
			// Launch Puppeteer
			const browser = await puppeteer.launch({ headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox'] });
			const page = await browser.newPage();
