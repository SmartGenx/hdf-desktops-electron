...
class ApplicantService {
	...
	async getApplicantReportByDirectorate(directorateId) {
		try {
			...
			const reports = Accredited.map((Accredited) => {
				...
				const diseaseNames = Accredited.Applicant.DiseasesApplicants.map((da) => da.Disease.name).join(', ');

				const dismissals = {
					totalAmount: Accredited.Dismissal.map((da) => da.totalAmount).reduce((acc, current) => acc + current, 0),

					approvedAmount: Accredited.Dismissal.map((da) => da.approvedAmount).reduce((acc, current) => acc + current, 0),
				};
				return new ApplicantByDirectorateViewModel(applicant, diseaseNames, Namedirectorate, state, dismissals, Namecategory);
			});
			return reports;
		} catch (error) {
			throw new DatabaseError('Error deleting accreditation.', error);
	...
	async getAllAccreditedWithParams(filterParams) {
		try {
			...
			if (!rest.squareId || isNaN(rest.squareId) || rest.squareId == '' || rest.squareId.equals == '') {
	...
	async getAllAccreditedAfterDismissal(filterParams) {
		...
		try {
			...
			const reports = dismissal?.map((dismissal) => {
				...
				const diseaseNames = dismissal.Accredited.applicant.diseasesApplicants.map((da) => da.Disease.name).join(', ');
	...
	async ApplicantByCategory(applicantfilter) {
		try {
			...
			const reports = Applicant.map((Applican) => {
				...
				return new ApplicantByCategoryViewModel(applicant, diseaseNames, Namedirectorate,categoryName);
			});
