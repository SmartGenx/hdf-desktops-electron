...
class AccreditedService {
  ...
  async filterAccreditedByDateAndLocation(squareId, location, docter, state) {
    try {
      ...
      throw new DatabaseError(
        "Error retrieving filtered accredited records.",
        error
      );
    }
  }
  ...
  async updateAccreditation(id, accreditedData) {
    try {
      ...
      const formNumber = accreditedData.formNumber ? +accreditedData.formNumber : existingAccreditation.formNumber;
      const numberOfRfid = accreditedData.numberOfRfid ? +accreditedData.numberOfRfid : existingAccreditation.numberOfRfid;
  ...
  async  AccreditedByPrescriptionServer(dataFilter) {
    try {
      ...
      const reports = Accredited.map((accredited) => {
        ...
        const prescription = {
          latestPrescriptionDate: accredited.prescription.map(
            (da) => new Date(da.prescriptionDate)
          )
            .sort((a, b) => a - b)
            .pop()
            .toISOString()
          ...
          renewalDate: accredited.prescription.map((da) => new Date(da.renewalDate))
            .sort((a, b) => a - b)
            .pop()
            .toISOString()
            .split("T")[0],
