...
async createDismissal(DismissalData) {
  ...
  try {
    const accredited=await this.prisma.dismissal.findFirst({where:{accreditedGlobalId:data.accreditedGlobalId}})
    if(!accredited){
    ...
    const checkdismissals=await this.prisma.dismissal.findFirst({where:{accreditedGlobalId:data.accreditedGlobalId,month:currentMonthStr}})

    if(!checkdismissals){
      const checkdismissal=await this.prisma.dismissal.findFirst({where:{accreditedGlobalId:data.accreditedGlobalId}})
...
//////////////////////////////////////////////////////////////////////////
  ...
  async updateDismissal(id, dismissalData) {
    try {
      ...
      const totalAmount = dismissalData.totalAmount ? +dismissalData.totalAmount : existingDismissal.totalAmount;
      const amountPaid = dismissalData.amountPaid ? +dismissalData.amountPaid : existingDismissal.amountPaid;
      const approvedAmount = dismissalData.approvedAmount ? +dismissalData.approvedAmount : existingDismissal.approvedAmount;
      return await this.prisma.dismissal.update({
        where: { globalId: id },
        data: {
          ...dismissalData,
